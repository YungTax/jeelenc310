(function($){
    ...
    var PLS_Metabox = {
        initialize: function() {
            PLS_Metabox.meta_box_tab();
            PLS_Metabox.required_field();
            PLS_Metabox.widget_select2_process();
        },
        meta_box_tab: function() {
           
			var tabBoxes 			= jQuery(pls_meta_data.meta_box_ids),
			...
			metabox_wrap_html 		= '<div class="pls-meta-tabs-wrap postbox"><div class="postbox-header"><h2 class="hndle ui-sortable-handle">'+ pls_meta_data.meta_box_title +'</h2><div class="handle-actions hide-if-no-js"><button type="button" class="handlediv" aria-expanded="true"><span class="screen-reader-text">Toggle panel: PLS Options</span><span class="toggle-indicator" aria-hidden="true"></span></button></div></div><div id="pls-tabbed-meta-boxes"></div></div>';
			;
			var meta_boxes = pls_meta_data.meta_box_ids.split(",");
			...
			jQuery(tabBoxes).appendTo('#pls-tabbed-meta-boxes');
            ...
            var menu_html 		= '<ul id="pls-meta-box-tabs" class="clearfix">\n';
            var total_hidden 	= 0;
            ...
            jQuery('#pls-tabbed-meta-boxes').before(menu_html);
            jQuery('#pls-meta-box-tabs a:first').addClass('active');

            jQuery('.editor-tab0').addClass('active').show();

            jQuery('.pls-meta-tabs-wrap').on('click', '.handlediv', function() {
            ...
            jQuery('#pls-meta-box-tabs li').on('click', 'a', function(event) {
				event.preventDefault();
                jQuery(tabBoxes).removeClass('active').hide();
                jQuery('#pls-meta-box-tabs a').removeClass('active');
        ...
        widget_select2: function(event, widget) {
            if (typeof (widget) == "undefined") {
                $('#widgets-right select.widget-select2:not(.select2-ready)').each(function(){
                    PLS_Metabox.widget_select2_item(this);
                });
            }
            else {
        ...
        widget_select2_process: function() {
            $(document).on('widget-added', PLS_Metabox.widget_select2);
            $(document).on('widget-updated', PLS_Metabox.widget_select2);
            PLS_Metabox.widget_select2();
        }
    };
    $(document).ready(function(){
        PLS_Metabox.initialize();
    });   
