 ...
 * Ajax search
 */
if( !class_exists( 'PLS_Ajax_Search' ) ){
	class PLS_Ajax_Search {

		public function __construct() {
			add_action( 'wp_ajax_pls_ajax_search', array( $this, 'ajax_suggestions' ) );
			add_action( 'wp_ajax_nopriv_pls_ajax_search', array( $this, 'ajax_suggestions' ) );
		}
		...
		function ajax_search_products ( $args ) {
			...
			$args['s'] 			= apply_filters( 'pls_ajax_search_products_search_query', esc_attr( $_REQUEST['query'] ) );
			...
			$query_function = apply_filters( 'pls_ajax_search_function', 'get_posts', $search_query, $args, $defaults );
		...
		function ajax_search_products_by_sku ( $args ) {
			$query = apply_filters( 'pls_ajax_search_products_by_sku_search_query', esc_attr( $_REQUEST['query'] ) );
		...
		public function ajax_search_posts ( $args ) {
			...
			$args['s'] 			= apply_filters( 'pls_ajax_search_query', esc_attr( $_REQUEST['query'] ) );
		...
		function ajax_suggestions () {
			...
			if ( ! wp_verify_nonce( $_POST['nonce'], 'pls_nonce' ) ) {
				$no_results = esc_html__( 'No data found.', 'pls-theme' ) ;
				$suggestions[] = array(
					'id' 	=> - 1,
					'value' => $no_results,
					'url' 	=> '',
			...
			$query 					= apply_filters( 'pls_ajax_search_query', esc_attr( $_REQUEST['query'] ) );
			$products 				= array();
			$posts 					= array();
			$sku_products 			= array();
			...
			$post_category 			= array();
			$post_cat 				= array();
			$support_post_type 		= pls_get_option( 'search-content-type', 'all' );
			$product_search_by_sku	= pls_get_option('product-search-by-sku',0);
			$args 					= apply_filters('pls_ajax_search_default_args', array(
				's' 					=> $query,
				'orderby' 				=> '',
				'post_type' 			=> array(),
				'post_status' 			=> 'publish',
			...
			if( $support_post_type == 'all' || $support_post_type == 'product' ){
				if ( PLS_WOOCOMMERCE_ACTIVE ) {
					...
					if( empty( $_REQUEST['product_cat'] ) ){					
						if( apply_filters( 'pls_ajax_product_cat_search', true ) ){
							$product_categories = $this->ajax_search_products_by_category();
						}
						if( apply_filters( 'pls_ajax_product_brand_search', true ) ){
			...
			if ( empty( $_REQUEST['product_cat']) ) {
				if($support_post_type == 'all' || $support_post_type == 'post'){
					...
					if(apply_filters('pls_ajax_post_category_search',true)){
						$post_category = $this->ajax_search_post_by_category(  );
					}
					if(apply_filters('pls_ajax_post_tag_search',true)){
			...
			foreach ( $results as $key => $post ) {
				if ( $post->post_type == 'product' && PLS_WOOCOMMERCE_ACTIVE ) {
					global $product;
					...
					if( ! $product_exist ){
						...
						if( pls_get_option( 'header-search-image', 1 ) ){
							$product_data['img'] = $product_image[0];
						}
						if( pls_get_option( 'header-search-price', 1 ) ){
							$product_data['price'] = ( ! is_user_logged_in() && pls_get_option( 'login-to-see-price', 0 ) ) ? '' : $product->get_price_html();
						}
						if( pls_get_option( 'header-search-rating', 0 ) && wc_review_ratings_enabled() ){
							$product_data['rating'] = wc_get_rating_html( $average, $rating_count );
						}
						$suggestions[] = apply_filters( 'pls_ajax_suggestion_args', $product_data );
					}
				} else {
					$suggestions[] 	= apply_filters('pls_ajax_suggestion_args', 
						array(
							'type' 		=> 'Page',
							'id' 		=> $post->ID,
			...
			if ( empty( $suggestions ) ) {
				$no_results = PLS_WOOCOMMERCE_ACTIVE ? esc_html__( 'No products found.', 'pls-theme' ) : esc_html__( 'No matches found', 'pls-theme' );
				$suggestions[] = array(
					'id' 	=> - 1,
					'value' => $no_results,
					'url' 	=> '',
	...
	$obj_pls_ajax_earch = new PLS_Ajax_Search();
}