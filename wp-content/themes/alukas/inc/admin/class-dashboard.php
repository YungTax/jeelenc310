...
class PLS_Dashboard {
	...
	function __construct() {
        ...
        $this->doc_link 			= 'https://docs.presslayouts.com/'.PLS_THEME_SLUG;
        $this->demo_preview_link 	= 'https://'.PLS_THEME_SLUG.'.presslayouts.com';
	...
	public function is_pls_server_available(){
		global $wp_version;
		...
		if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
			...
			$actions = sprintf( '<a href="%1$s" title="%2$s">Activate</a>',
				esc_url( add_query_arg( array(
					...
					admin_url( 'admin.php?page=pls-theme' ) ) ),
				$item['sanitized_plugin'] );
			...
			$actions = sprintf( '<a href="%1$s" title="%2$s">Update</a>',
				wp_nonce_url( add_query_arg( array(
					...
					'tgmpa-nonce' ),
				$item['sanitized_plugin'] );
		} elseif ( pls_check_plugin_active( $item['file_path'] ) ) {
