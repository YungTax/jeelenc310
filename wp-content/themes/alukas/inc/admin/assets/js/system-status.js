...
jQuery( function ( $ ) {
	...
	var wcSystemStatus = {
		...
		generateReport: function() {
			...
			$( '#pls-system-status .pls-dash-title,#pls-system-status table tbody' ).each( function() {
				if ( $( this ).hasClass( 'pls-dash-title' ) ) {
					var label =  $( this ).text();
                    report = report + '\n### ' + $.trim( label ) + ' ###\n\n';
				} else {
			...
			try {
				$( '#pls-debug-report' ).slideDown();
				$( '#pls-debug-report' ).find( 'textarea' ).val( '`' + report + '`' ).focus().select();
				$( this ).fadeOut();
				return false;
			} catch ( e ) {
				/* jshint devel: true */
