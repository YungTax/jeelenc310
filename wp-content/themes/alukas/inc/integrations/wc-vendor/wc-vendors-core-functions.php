...
add_action( 'init','pls_wc_hook' );

function pls_wc_hook(){
	if ( get_option('wcvendors_display_label_sold_by_enable') == 'yes' ) {
		...
		if($sold_by_template == 'theme'){
			...
			add_action( 'pls_woocommerce_shop_loop_item_title', 'pls_wc_loop_sold_by_label', 21 );
			add_action( 'woocommerce_single_product_summary', 'pls_wc_item_sold_by_label',8 ); 
	...
	add_filter( 'body_class', 'pls_wc_body_class' );
}

function pls_wc_body_class( $classes ) {
		if ( class_exists( 'WC_Vendors' ) ) {
			...
			if ( is_page( $orders_page_id ) ) {
				$classes[] = 'pls-wc-vendors';
			} elseif ( $shop_settings_page == get_the_ID() ) {
				$classes[] = 'pls-wc-vendors wc-vendors-shop-settings';
			} elseif( $shop_dashboard_page == get_the_ID() ){ 
				$classes[] = 'pls-wc-vendors wc-vendors-dashboard';
			}
		}
...
function pls_wc_loop_sold_by_label(){
	$sold_by_loop = pls_get_option( 'enable-sold-by-in-loop' , 1 );
	if( !$sold_by_loop ) { return false; }
	pls_get_wc_vendor_name();
}

function pls_wc_item_sold_by_label(){
	$sold_by_single = pls_get_option( 'enable-sold-by-in-single' , 1 );
	if( !$sold_by_single ) { return false; }
	pls_get_wc_vendor_name();
}

function pls_get_wc_vendor_name(){
	...
	$sold_by_label 	= apply_filters('wcvendors_sold_by_in_loop',esc_html__( 'Sold By : ', 'pls-theme' ));
	?>
